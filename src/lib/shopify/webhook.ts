 ...
 */
export async function registerShopifyProductsWebhook(
  accessToken: string,
  shopDomain: string
): Promise<string> {
    ...
    `https://${shopDomain}/admin/api/2024-01/webhooks.json`,
    {
      ...
      body: JSON.stringify({
        webhook: {
          ...
          address: `${process.env.NEXT_PUBLIC_APP_URL}/api/webhooks/shopify-price`,
          format: 'json',
        },
      }),
    }
  ...
  if (!res.ok) {
    if (res.status === 422) {
      console.log(`[Shopify] Products webhook already registered for ${shopDomain}`)
      return 'already_registered'
    }
    const body = await res.text()
    throw new Error(`Shopify products/update webhook registration failed for ${shopDomain}: ${body}`)
  }
